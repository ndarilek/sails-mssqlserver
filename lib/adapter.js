...
module.exports = (function () {
  ...
  var adapter = {
    ...
    registerDatastore: function (datastoreConfig, models, cb) {
      if (!datastoreConfig.identity) return cb(new Error('Connection is missing an identity.'))
      if (datastores[datastoreConfig.identity]) return cb(new Error('Datastore is already registered.'))

      // Add in logic here to initialize datastore
     ...
     * @param {Function} cb
     */
    connectDatastore: function (datastore, cb) {
      var uniqId = _.uniqueId()
      var persistent = !!datastore.config.persistent
      var assigned = !!datastore.mssqlConnection
      var connected

      if (persistent) {
        connected = assigned && !!datastore.mssqlConnection.connected
      } else {
        connected = assigned && datastore.mssqlConnection[uniqId] && datastore.mssqlConnection[uniqId].connected
      }

      if (persistent && !connected) {
        datastore.mssqlConnection = new mssql.Connection(marshalConfig(datastore.config), cb)
      } else if (!persistent && !connected) {
        if (!assigned) {
          datastore.mssqlConnection = []
        }

        datastore.mssqlConnection[uniqId] = new mssql.Connection(marshalConfig(datastore.config), function (err) {
          cb(err, uniqId)
        })
      } else {
        _.defer(cb)
    ...
    find: function (dataStoreName, query, cb) {
      ...
      var schemaName = getSchemaName(query)
      var tableName = '[' + schemaName + ']' + '.[' + query.using + ']'
      var statement = sql.selectQuery(tableName, query.criteria)

      adapter.connectDatastore(datastore, function __FIND__ (err, uniqId) {
        if (err) {
        ...
        var mssqlConnect
        if (!uniqId) {
          mssqlConnect = datastore.mssqlConnection
        } else {
          mssqlConnect = datastore.mssqlConnection[uniqId]
        }
  ...
  function getSchemaName (query) {
    return query.meta && query.meta.schemaName ? query.meta.schemaName : 'dbo'
  }
